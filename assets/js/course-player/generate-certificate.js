...
(function ($) {
  $(document).ready(function () {
    $('body').on('click', '.masterstudy-course-player-course-completed__buttons .masterstudy-button, .masterstudy_preview_certificate', function (e) {
      ...
      if (typeof $(this).attr('data-user_id') !== 'undefined') {
        user_id = $(this).attr('data-user_id');
      }
      if (id || courseId) {
        getCertificate(id, courseId, user_id);
      }
    });
  });
  function getCertificate(id, courseId, user_id) {
    var courseId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
    var url = course_certificate.ajax_url + '?action=stm_get_certificate&nonce=' + course_certificate.nonce + '&post_id=' + id + '&course_id=' + courseId + '&user_id=' + user_id;
    $.ajax({
      url: url,
      method: 'get',
